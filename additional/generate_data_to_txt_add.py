            ...
            if (vector[0] == record[0] and len(record) > 2):
                ...
                if (int(record[1]) > 1):
                    for n_record in range(1, num_record):
                        ...
                        record_vector = [ next_isChange, record[0], curr_isChange]
                        record_vector.extend(record[current_updateTime_index:current_updateTime_index + 4]) # update time + citation + h_index + i10_index
...
if (word_or_vector == 1):
    filename = date + "/dataRecord_vector_add.txt"
    read_word_or_vector_file = date + "/vector_withID.txt"
else:
    filename = date + "/dataRecord_word_add.txt"
